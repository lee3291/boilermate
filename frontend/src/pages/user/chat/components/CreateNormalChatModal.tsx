...
interface CreateNormalChatModalProps {
    isOpen: boolean;
    onClose: () => void;
    currentUserId: string;
    onSearchUsers: (query: string) => Promise<User[]>; // API search
    onCreateChat: (participantId: string) => Promise<void>; // Create 1-on-1 chat
}

export default function CreateNormalChatModal({
                                                  isOpen,
                                                  onClose,
                                                  currentUserId,
                                                  onSearchUsers,
                                                  onCreateChat,
                                              }: CreateNormalChatModalProps) {
    ...
    const handleCreate = async () => {
        ...
        setIsCreating(true);
        try {
            await onCreateChat(selectedUser.id); // Create normal 1-on-1 chat
