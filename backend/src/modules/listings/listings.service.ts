...
export class ListingsService {
  ...
  async create(
    ...
    try {
      ...
      if (author) {
        // --- EMAIL NOTIFICATION ---
        // When a new listing is created, find all users who follow the author
        // and send them an email notification, but only if they have opted in.
        const follows = await this.prisma.follow.findMany({
          where: { followingId: author.id },
          include: {
            follower: {
              include: {
                // Include the follower's settings to check their notification preferences.
                settings: true,
              },
            },
          },
        ...
        if (follows.length > 0) {
          // Filter followers who have enabled the 'email_on_follow_new_listing' setting.
          // If the setting is not defined (e.g., for older users), default to true to maintain old behavior.
          ...
          // Send the email to the filtered list of followers.
