...
export class ListingsService {
    ...
    private normalizeCreateInput(input: LegacyCreateListingDto | CreateListingDetails) {
        const price =
        ...
        const parseMaybeDate = (v: any) => (v == null ? null : typeof v === 'string' ? new Date(v) : (v as Date));

        const roommates =
            Number.isInteger((input as any).roommates) && (input as any).roommates >= 1 ? (input as any).roommates : undefined;
    ...
    async create(input: CreateListingDetails | LegacyCreateListingDto): Promise<{ listing: ListingResponse }> {
    ...
    async unsaveListing(args: { listingId: string; username: string }) {
        ...
        return { listingId: args.listingId, username: args.username, isSaved: false as const };
    }
    ...
    async listSavedBy(args: { listingId: string; page: number; pageSize: number }) {
    ...
    async listingsSavedByUser(args: { username: string; page: number; pageSize: number }) {
    ...
    async update(listingID: string, dto: Partial<LegacyCreateListingDto>) {
        ...
        if (typeof (dto as any).pricing === 'number') patch.pricing = (dto as any).pricing;
        if ('mediaUrls' in dto) patch.mediaUrls = dto.mediaUrls ?? [];
        if ('moveInStart' in dto)
            patch.moveInStart =
                dto.moveInStart == null ? null : typeof dto.moveInStart === 'string' ? new Date(dto.moveInStart) : (dto.moveInStart as Date);
        if ('moveInEnd' in dto)
            patch.moveInEnd =
                dto.moveInEnd == null ? null : typeof dto.moveInEnd === 'string' ? new Date(dto.moveInEnd) : (dto.moveInEnd as Date);
    ...
    async findByUser(username: string, include?: ('ACTIVE' | 'INACTIVE' | 'ARCHIVED')[]) {
        const statuses = include && include.length ? include : (['ACTIVE', 'INACTIVE', 'ARCHIVED'] as const);
        const rows = await this.prisma.listing.findMany({
            where: {
                user: username,
